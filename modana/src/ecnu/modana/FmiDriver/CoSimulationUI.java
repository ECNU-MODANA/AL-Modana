...
//			logger.debug("model opened!");
    ...
    //verifier========================================================================
    private void initVerifierPanel(BorderPane verifierPane) {
		SplitPane verCenterPane = new SplitPane();
		verifierPane.setCenter(verCenterPane);
		BorderPane verTopPane = new BorderPane();
		verTopPane.setPrefHeight(450);
		verTopPane.setMaxHeight(450);
		verTopPane.setMinHeight(450);
		initVerTopPane(verTopPane);
 		BorderPane verBottomPane = new BorderPane();
 		verCenterPane.setDividerPositions(0.1f);
 		verCenterPane.setOrientation(Orientation.VERTICAL);
 		verCenterPane.getItems().addAll(verTopPane,verBottomPane);
		SplitPane verCenterBPane = new SplitPane();
		verTopPane.setCenter(verCenterBPane);
 		BorderPane verLeftPane = new BorderPane();
 		BorderPane verRightPane = new BorderPane();
 		verLeftPane.setPrefWidth(300);
 		verLeftPane.setMaxWidth(300);
 		verLeftPane.setMinWidth(300);
 		initVerLeftPane(verLeftPane);
 		verCenterBPane.getItems().addAll(verLeftPane, verRightPane);
 		verCenterBPane.setDividerPositions(0.1f);
	}
	
	   private void initVerLeftPane(BorderPane verLeftPane) {
		   VBox algorithmBox = new VBox();
		   Label smcMaLable = new Label();
		   smcMaLable.setText("select SMC algorithm");
		   smcMaLable.setMinWidth(200);
		   ChoiceBox smcMa = new ChoiceBox(FXCollections.observableArrayList(  
	        		"SSP","SPRT", "BHT", "APMC", "BIE", "AL-SMC")  
	        	);  
		   smcMa.setMinWidth(170);
		   smcMa.getSelectionModel().select(0);
		   algorithmBox.getChildren().addAll(smcMaLable,smcMa);
		   verLeftPane.getChildren().add(algorithmBox);
	}
	private void initVerTopPane(BorderPane topPane) {
		    VBox menuPane = new VBox();
		 	initVerMenu(menuPane);
		 	topPane.setTop(menuPane);
		 	
	 }
	   

	private void initVerMenu(VBox menuPane) {
		menuBar = new MenuBar();
        ...
        MenuItem parameter = new MenuItem("Parameters Setting");
