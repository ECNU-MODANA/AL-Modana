 ...
 */
public class BBallPlug2 extends ToolSlave{

    double h=4,g=9.8,v=0,c=-0.8, effect = 1.0;
    int eventNum = 0;

    public BBallPlug2(double h, double v ,double c){
        this.v = v;
        this.h = h;
        this.c = c;
    }
...
//        v *= effect;
        ...
        if (calculatedHeight > 0.00001) {
            v = v + g* stepSize;
            h = calculatedHeight;
            return stepSize;
        } else if (calculatedHeight < -0.00001) {
            return -1;
        } else {
            v = -Math.abs((v + g * stepSize)*c);
            h = 0.0;
        }
        return stepSize;
    }
    ...
    public double Predict(){
        ...
        //0.0步长，从而使得仿真“终止”；
            return (Math.sqrt(v*v+2*g*h)-v)/g;
