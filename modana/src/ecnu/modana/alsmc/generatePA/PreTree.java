...
import ecnu.modana.alsmc.util.selectData;
...
public class PreTree {
	...
	static int con = 0;
	public static void createPreTree() {
		for (int i = 0; i < selectData.traPAList.size(); i++) {
			currentNode = root;
			List<TreeNode> nodeList = selectData.traPAList.get(i);
			root.n++;
			if (nodeList.get(nodeList.size() - 1).tcheck == 0) {
				root.f++;
			} else {
