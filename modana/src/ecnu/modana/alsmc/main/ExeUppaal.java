...
import ecnu.modana.alsmc.pca.PCA;
...
public class ExeUppaal {
		...
		try {
			for (int m = 0; m < 1; m++) {
				...
				for (int h = 0; h < n; h++) {
					...
					if (UserFile.praNameList!=null&&!UserFile.praNameList.equals("")) {
	...
	public static void generateNewTrace() {
		...
		//运行uppaal
		try {
			String st = UserFile.verifytaPath+" "+UserFile.markovPath+" "+UserFile.fmuPath;
