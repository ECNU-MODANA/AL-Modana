...
//		Server.log.println("numChoices:"+numChoices+" "+currentState.toString());
//		Server.log.flush();
		if (numChoices == 0)
			return false;
	 ...
	 * Added by me
	 */
	public String GetCurState() throws Exception
	{
		//return currentState.toString();
		return path.getCurrentState().toString();
	}
	public void SetValue(int i,Object val)
	{
		currentState.setValue(i, val);
		path.getCurrentState().setValue(i, val);
	}
	/**
	 * Select, at random, n successive transitions and execute them.
	 * For continuous-time models, the time to be spent in each state before leaving is also picked randomly.
	...
	public int automaticTransitions(int n) throws PrismException
	{
		...
		{
			//Server.log.println("cur:"+currentState.toString());
			//this.createNewPath(modulesFile);
			//this.initialisePath(currentState);
			updater.calculateStateRewards(currentState, tmpStateRewards);
			// Initialise stored path
			path.initialise(currentState, tmpStateRewards);
			// Reset transition list
	 ...
	 * The function returns the number of transitions successfully taken. 
	 */
	public int automaticTransitions() throws PrismException
	{
		int i = 0;
		if(null!=currentState)
		{
