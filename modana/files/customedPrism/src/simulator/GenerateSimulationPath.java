...
//		if(null==initialState) initialState=new State(3);
//		initialState.setValue(0, "0");
//		initialState.setValue(1, "0");
//		initialState.setValue(2, "2");
		this.modulesFile = modulesFile;
		this.initialState = initialState;
		...
		int split=details.lastIndexOf("values=");
		if(split>=0)
		{
			String tStr=details.substring(split+1,details.length());
			tStr=tStr.substring(6,tStr.length());
			details=details.substring(0,split-1);
			String[]tv=tStr.split(",");
			if(null==initialState&&tv.length>0) 
			{
				initialState=new State(tv.length);
				for(int i=0;i<tv.length;i++)
					initialState.setValue(i, tv[i]);
				this.initialState=initialState;
				//throw new PrismLangException("get values error: "+details+" "+tStr+" "+tv[0], null);
			}
		}
		parseDetails(details);
		PathDisplayer displayer = generateDisplayerForExport();
